...
// Constructor
//----------------------------------------------------------------
CoinFTBus::CoinFTBus(const std::string& port, unsigned int baud_rate,
    ...
    : serial(io), running(false), packet_size(0), num_raw_channels(0) 
{
    ...
    if (serial.is_open()) {
        ...
        serial.close();
        std::this_thread::sleep_for(std::chrono::milliseconds(10)); // Allow OS time to release port
    }
    ...
    for (const auto& config : sensorConfigs) {
        ...
        // Create ONNX session
        sensor.session = std::make_unique<Ort::Session>(ort_env, model_file.c_str(), session_options);

        // Retrieve input and output node names
        sensor.input_name = sensor.session->GetInputNameAllocated(0, allocator).get();
        sensor.output_name = sensor.session->GetOutputNameAllocated(0, allocator).get();
...
CoinFTBus::~CoinFTBus() {
    ...
    {
        std::lock_guard<std::mutex> lock(instanceMutex);
        activeInstances.erase(std::remove(activeInstances.begin(), activeInstances.end(), this), activeInstances.end());
    }
}
...
void CoinFTBus::initializeBus() {
    std::cout << "[DEBUG] Attempting to reset device to IDLE mode..." << std::endl;
...
void CoinFTBus::stopStreaming() {
    if (!running.exchange(false)) return;  // Prevent multiple calls
    ...
    if (data_thread.joinable()) {
        std::thread killer([&]() {
            ...
            if (data_thread.joinable()) {
                std::cerr << "[WARNING] Data thread did not stop in time, forcefully detaching!" << std::endl;
                data_thread.detach();
            }
        });
...
void CoinFTBus::tareSensor(int address) {
    ...
    if (it != sensors.end()) {
        ...
        std::cout << "Taring initiated for sensor at address: " << address << std::endl;
    } else {
        std::cerr << "Sensor with address " << address << " not found." << std::endl;
    }
}
...
std::vector<uint8_t> CoinFTBus::readData(size_t length) {
    ...
    timer.async_wait([&](const boost::system::error_code& error) {
        if (!error && !read_complete) {  // Timer expired
            timeout_flag = true;
            std::cerr << "[ERROR] Serial read timeout! Cancelling operation." << std::endl;
            serial.cancel();
        }
    });
    ...
    boost::asio::async_read(serial, boost::asio::buffer(buffer, length),
        [&](const boost::system::error_code& error, std::size_t bytes_transferred) {
            ...
            if (error) {
                std::cerr << "[ERROR] Serial read failed: " << error.message() << std::endl;
            } else {
    ...
    if (timeout_flag) {
        std::cerr << "[ERROR] Read operation timed out. Returning empty buffer." << std::endl;
        return {};
    }
    ...
    while (running.load()) {
        try {
            ...
            if ((end_byte & 0x0F) != ETX) {
                std::cerr << "[ERROR] Bad end framing byte. Expected ETX (0x03), but got: 0x"
                        << std::hex << static_cast<int>(end_byte & 0x0F) << std::dec << std::endl;
            ...
            // Process the sensor's data.
            {
                ...
                if (it == sensors.end()) {
                    std::cerr << "Received data for unknown sensor address: " << sensorAddress << std::endl;
                    continue;
                }
                ...
                if (sensor.tareInProgress) {
                    ...
                    if (sensor.tareSampleCount >= sensor.tareSampleTarget) {
                        ...
                        sensor.tareOffset = sum / static_cast<double>(sensor.tareSampleCount);
                        sensor.tareSamples.clear();
                        sensor.tareSampleCount = 0;
                        sensor.tareInProgress = false;
                        std::cout << "Taring completed for sensor " << sensorAddress << std::endl;
                    }
                ...
                auto output_tensors = sensor.session->Run(Ort::RunOptions{nullptr},
                                                            input_names.data(),
                                                            &input_tensor,
                                                            1,
                                                            output_names.data(),
            ...
            std::cerr << "Exception in data acquisition loop: " << e.what() << std::endl;
            running.store(false);
        }
    }
}
...
void CoinFTBus::signalHandler(int signum) {
    if (signum == SIGINT || signum == SIGTERM) {
        std::cout << "\n[DEBUG] Signal (" << signum << ") received, shutting down all active instances..." << std::endl;
        {
            std::lock_guard<std::mutex> lock(instanceMutex);
        ...
        std::cout << "\n[DEBUG] Signal (SIGTSTP) received, suspending all instances..." << std::endl;
